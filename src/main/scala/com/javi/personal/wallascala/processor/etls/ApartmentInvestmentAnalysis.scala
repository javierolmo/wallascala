...
case class ApartmentInvestmentAnalysis(config: ProcessorConfig, override val dataSourceProvider: DataSourceProvider = new DefaultDataSourceProvider())(implicit spark: SparkSession) extends Processor(config, dataSourceProvider) {
  ...
  private lazy val properties = dataSourceProvider.readProcessed(ProcessedTables.WALLAPOP_PROPERTIES, Some(config.date))
  private lazy val postalCodeAnalysis = dataSourceProvider.readProcessed(ProcessedTables.PROPERTIES, Some(config.date))
